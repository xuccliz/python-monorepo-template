...
from modeler.models.spline import build_spline_model
...
def snapshot() -> OptionSurfaceSnapshot:
    ...
    for k in strikes:
        time_value = 2.0 * np.exp(-0.005 * (abs(100 - k)) ** 2)
...
def test_spline_smoke(snapshot: OptionSurfaceSnapshot) -> None:
    """Test that Spline model builds and runs."""
    T = 30 / 365.0
    model = build_spline_model(snapshot=snapshot, T=T)
    assert model is not None
    assert model.fit.n_points >= 4


def test_spline_prob_above(snapshot: OptionSurfaceSnapshot) -> None:
    """Test probability estimation."""
    T = 30 / 365.0
    model = build_spline_model(snapshot=snapshot, T=T)
    assert model is not None

    # ITM call / OTM put -> Low strike -> Prob > 0.5
...
def test_spline_insufficient_data() -> None:
