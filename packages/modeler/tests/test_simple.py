...
class TestEstimateProbabilitySimple:
    ...
    def snapshot(self) -> OptionSurfaceSnapshot:
        ...
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
    ...
    def test_equal_call_put_gives_50_percent(self, snapshot: OptionSurfaceSnapshot) -> None:
        result = estimate_probability_simple(snapshot=snapshot, strike_price=100.0)

        assert result is not None
        assert result == pytest.approx(0.5)

    def test_higher_call_gives_higher_prob_above(self) -> None:
        """Call worth more than put -> higher probability of finishing above strike_price."""
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=7.0, ask=8.0, mid=7.5, spread=1.0),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=2.0, ask=3.0, mid=2.5, spread=1.0),)
    ...
    def test_higher_put_gives_lower_prob_above(self) -> None:
        """Put worth more than call -> lower probability of finishing above strike_price."""
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=2.0, ask=3.0, mid=2.5, spread=1.0),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=7.0, ask=8.0, mid=7.5, spread=1.0),)
        snapshot = OptionSurfaceSnapshot(symbol=TEST_SYMBOL, expiration_date=TEST_EXPIRY, calls=calls, puts=puts)
    ...
    def test_strike_not_found_returns_none(self, snapshot: OptionSurfaceSnapshot) -> None:
        result = estimate_probability_simple(snapshot=snapshot, strike_price=999.0)
        assert result is None

    def test_zero_call_mid_returns_none(self) -> None:
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=0.0, ask=0.0, mid=0.0, spread=0.0),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
        snapshot = OptionSurfaceSnapshot(symbol=TEST_SYMBOL, expiration_date=TEST_EXPIRY, calls=calls, puts=puts)
    ...
    def test_zero_put_mid_returns_none(self) -> None:
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=0.0, ask=0.0, mid=0.0, spread=0.0),)
        snapshot = OptionSurfaceSnapshot(symbol=TEST_SYMBOL, expiration_date=TEST_EXPIRY, calls=calls, puts=puts)
    ...
    def test_max_spread_filter_rejects_wide_call(self) -> None:
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=4.0, ask=6.0, mid=5.0, spread=2.0),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
        snapshot = OptionSurfaceSnapshot(symbol=TEST_SYMBOL, expiration_date=TEST_EXPIRY, calls=calls, puts=puts)

        result = estimate_probability_simple(snapshot=snapshot, strike_price=100.0, max_spread=1.0)
        assert result is None

    def test_max_spread_filter_rejects_wide_put(self) -> None:
        calls = (OptionPoint(strike_price=100.0, option_type="call", bid=4.8, ask=5.2, mid=5.0, spread=0.4),)
        puts = (OptionPoint(strike_price=100.0, option_type="put", bid=4.0, ask=6.0, mid=5.0, spread=2.0),)
        snapshot = OptionSurfaceSnapshot(symbol=TEST_SYMBOL, expiration_date=TEST_EXPIRY, calls=calls, puts=puts)
