...
from domain.models import ConfidenceDiagnostics, OptionSurfaceSnapshot
...
def compute_confidence(
    *,
    ...
    strike_price: float,
    prob_simple: Probability | None = None,
    prob_slope: Probability | None = None,
    prob_svi: Probability | None = None,
    prob_spline: Probability | None = None,
    ...
    strike_price:
        Target strike K
    prob_simple:
        Probability from simple estimator (call/put ratio)
    prob_slope:
        Probability from slope-based estimator
    prob_svi:
        Probability from SVI model
    prob_spline:
        Probability from spline model
    max_relative_spread:
    ...
    probs = [p for p in [prob_simple, prob_slope, prob_svi, prob_spline] if p is not None]

    if len(probs) < 2:
        agreement_score = 0.0
    else:
        # max pairwise difference
        max_delta = max(abs(a - b) for i, a in enumerate(probs) for b in probs[i + 1 :])
        # exponential decay: small differences are fine, large ones kill confidence
        agreement_score = exp(-5.0 * max_delta)
    ...
    # ------------------------------------------------------------
    # 3. Local monotonicity sanity (calls ↓ as strike price ↑)
    # ------------------------------------------------------------
    ...
    i: int | None = None
    try:
        i = min(range(len(strikes)), key=lambda j: abs(strikes[j] - strike_price))
    except ValueError:
        pass
    ...
    # ------------------------------------------------------------
    # 4. Strike price spacing sanity (avoid extrapolation)
    # ------------------------------------------------------------
