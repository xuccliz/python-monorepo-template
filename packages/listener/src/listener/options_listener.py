...
class OptionsQuoteListener:
    def __init__(
        self,
        *,
        event_queue: asyncio.Queue[OptionQuoteEvent],
        ...
        Args:
            event_queue: Queue to push quote events to (required).
            ...
            tickers: Underlying tickers to fetch options for.
                    Defaults to SYMBOLS from domain.types.
        """
    ...
    async def _handle_message(self, messages: list[WebSocketMessage]) -> None:
        for msg in messages:
            ...
            try:
                ...
                # Push to queue for downstream state builder
