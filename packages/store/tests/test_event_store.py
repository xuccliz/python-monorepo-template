...
# Test dates
DATE_JAN_10 = make_expiry_datetime("2026-01-10")
DATE_JAN_17 = make_expiry_datetime("2026-01-17")
...
class TestEventStore:
    ...
    def test_refresh_replaces_old_events(self, mock_fetch: MagicMock) -> None:
        ...
        mock_fetch.return_value = [
            EventMetadata(symbol="NVDA", question_id="old1", end_date=DATE_JAN_10),
        ]
        store = EventStore()
        store.refresh()
        ...
        mock_fetch.return_value = [
            EventMetadata(symbol="AAPL", question_id="new1", end_date=DATE_JAN_10),
            EventMetadata(symbol="TSLA", question_id="new2", end_date=DATE_JAN_10),
        ]
        store.refresh()
    ...
    def test_refresh_skips_events_without_symbol(self, mock_fetch: MagicMock) -> None:
        mock_fetch.return_value = [
            EventMetadata(symbol="NVDA", question_id="1", end_date=DATE_JAN_10),
            EventMetadata(symbol=None, question_id="2", end_date=DATE_JAN_10),  # No symbol
            EventMetadata(symbol="AAPL", question_id="3", end_date=DATE_JAN_10),
        ]
        store = EventStore()
...
class TestGetMarket:
    ...
    def store_with_markets(self) -> EventStore:
        """Store with events containing markets - populated via refresh mock."""
        events = [
            EventMetadata(
                symbol="NVDA",
                ...
                markets=[
                    MarketMetadata(
                        question="Will NVIDIA finish week of Jan 5 above $140?",
                        strike_price=140.0,
                        yes_price=0.65,
                        no_price=0.35,
                    ),
                    MarketMetadata(
                        question="Will NVIDIA finish week of Jan 5 above $150?",
                        strike_price=150.0,
                        yes_price=0.40,
                        no_price=0.60,
                    ),
            ...
            EventMetadata(
                ...
                markets=[
                    MarketMetadata(
                        question="Will NVIDIA finish week of Jan 12 above $145?",
                        strike_price=145.0,
                        yes_price=0.55,
                        no_price=0.45,
                    ),
        ...
        store = EventStore()
        with patch("store.event_store.fetch_stock_events", return_value=events):
            store.refresh()
        return store

    def test_get_market_returns_matching_market(self, store_with_markets: EventStore) -> None:
        market = store_with_markets.get_market("NVDA", DATE_JAN_10, 140.0)
    ...
    def test_get_market_returns_market_for_different_end_date(self, store_with_markets: EventStore) -> None:
        market = store_with_markets.get_market("NVDA", DATE_JAN_17, 145.0)
    ...
    def test_get_market_returns_none_for_empty_store(self) -> None:
        ...
        market = store.get_market("NVDA", DATE_JAN_10, 140.0)
...
class TestGetPolymarketProb:
    ...
    def store_with_markets(self) -> EventStore:
        ...
        events = [
            EventMetadata(
                ...
                markets=[
                    MarketMetadata(
                        question="Will NVIDIA finish above $140?",
                        strike_price=140.0,
                        yes_price=0.65,
                        no_price=0.35,
                    ),
                    MarketMetadata(
                        question="Will NVIDIA finish above $150?",
                        strike_price=150.0,
                        yes_price=0.40,
                        no_price=0.60,
                    ),
    ...
    def test_get_polymarket_prob_above_returns_yes_price(self, store_with_markets: EventStore) -> None:
        prob = store_with_markets.get_polymarket_prob("NVDA", DATE_JAN_10, 140.0, direction="above")

        assert prob == 0.65

    def test_get_polymarket_prob_below_returns_no_price(self, store_with_markets: EventStore) -> None:
        prob = store_with_markets.get_polymarket_prob("NVDA", DATE_JAN_10, 140.0, direction="below")

        assert prob == 0.35

    def test_get_polymarket_prob_returns_none_for_missing_market(self, store_with_markets: EventStore) -> None:
        prob = store_with_markets.get_polymarket_prob("NVDA", DATE_JAN_10, 999.0, direction="above")

        assert prob is None

    def test_get_polymarket_prob_returns_none_for_missing_symbol(self, store_with_markets: EventStore) -> None:
        prob = store_with_markets.get_polymarket_prob("MSFT", DATE_JAN_10, 140.0, direction="above")
